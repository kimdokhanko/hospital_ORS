...
public class PatientView {
	...
	public int mainMenu() {
		...
		System.out.println("3. 진료예약 변경하기");
	...
	public Patient reservationTrmt() {
		Scanner sc = new Scanner(System.in);
		System.out.println("*^^ 진료 예약을 변경합니다.^^*");
		System.out.print("진료과목([정형외과/내과] 입력가능) : ");
		String medicalCourse = sc.next();
		System.out.print("의사 선택([정형외과 = 김철수] [내과 = 김영희)] : ");
		String doctorName = sc.next();
	...
	public String inputPatientId(String search) {
		Scanner sc = new Scanner(System.in);
		System.out.print(search + "할 아이디 입력: ");
		String pId = sc.next();
		return pId;
	}

	public void showOne(Patient patient) {
		System.out.println("^.^ 진료 예약을 조회합니다.^.^");
		System.out.println("아이디 : " + patient.getPatientId());
		System.out.println("이름 : " + patient.getPatientName());
		System.out.println("생년월일 : " + patient.getPatientNo());
		System.out.println("진료과목 : " + patient.getMedicalCourse());
		System.out.println("의사 이름 : " + patient.getDoctorName());
		System.out.println("초진/재진(진료구분) : " + patient.getTreatmentDivision());
		System.out.println("진료 예약 날짜 : " + patient.getTreamentDate());
	}

	public Patient modifyPatient(String patientId) {
		Scanner sc = new Scanner(System.in);
		Patient patient = new Patient();
		patient.setPatientId(patientId);
